...
video_players.forEach(video_player => {
  ...
  <div class="controls active">
    ...
    <div class="controls-list">
      ...
      <span class="icon contorl_button" id="prevButton"><i class="fa-solid fa-backward-step"></i></span>
      <span class="icon">
          <i class="material-icons play_pause" >play_arrow</i>
        </span>
        <span class="icon contorl_button" id="nextButton"><i class="fa-solid fa-forward-step"></i></span>
...
function playAdjacentVideo(direction) {
    var currentVideoLink = document.querySelector('.playlist.video .video-link.playing');
    if (currentVideoLink) {
        var playlistId = currentVideoLink.getAttribute('data-playlist-id');
        var videoIndex = parseInt(currentVideoLink.getAttribute('data-video-index'));
        var newVideoIndex = videoIndex + direction;

        // Get the adjacent video link
        var adjacentVideoLink = document.querySelector(`.playlist.video .video-link[data-playlist-id="${playlistId}"][data-video-index="${newVideoIndex}"]`);
...
function playVideoFromLink(videoLink) {
    // Remove 'playing' class from all video links
    var videoLinks = document.querySelectorAll('.playlist.video .video-link');
    videoLinks.forEach(function (link) {
        link.classList.remove('playing');
    });

    // Add 'playing' class to the clicked video link
    videoLink.classList.add('playing');

    // Play the video
    var videoUrl = videoLink.getAttribute('data-url');
    var videoTitle = videoLink.getAttribute('data-title');
    var videoDescription = videoLink.getAttribute('data-description');
    var playlistId = videoLink.getAttribute('data-playlist-id');
    var videoIndex = videoLink.getAttribute('data-video-index');

    // Set the video source, update the title, and set the description
